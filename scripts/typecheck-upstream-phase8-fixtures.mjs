...
/** Type-check the Phase 8 persistence/compaction/pruner oracle against pinned upstream. */
...
const oraclePath = join(scriptDir, 'generate-upstream-phase8-fixtures.ts')
...
const trackedChanges = execFileSync(
  ...
  { cwd: upstreamRoot, encoding: 'utf8' },
).trim()
if (trackedChanges !== '') {
  throw new Error('typecheck requires a clean upstream tracked working tree')
}
...
if (diagnostics.length > 0) {
  ...
  process.stdout.write('upstream Phase 8 persistence/compaction/pruner oracle typecheck passed\n')
}
