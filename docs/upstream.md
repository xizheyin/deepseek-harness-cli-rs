...
never invents a successful `compaction/end` or a missing prune replacement.
Focused Rust tests also lock the frozen camel-case wire shape, reader-only
...
The output-usage anchor, summary dispatch, checkpoint construction, model-free
raw-row pruning, and the real Agent producer remain later Phase 8 slices.
...
producer, output-usage anchor, pruner, and deterministic comparison tests are
all present.
