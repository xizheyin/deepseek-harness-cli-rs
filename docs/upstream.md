...
The current Rust checkpoint implements the provider-neutral event and recovery
substrate, not the compaction producer. It has validated codecs for the four
...
interrupted bracket stages. Recovery reports an orphan bracket or prune marker,
closes only the enclosing step/turn facts that can be proved, and never invents
a successful `compaction/end` or a missing prune replacement. Focused Rust tests
also lock the frozen camel-case wire shape, reader-only legacy forms, resource
ceilings, ordinary-versus-recovery admission, restart from each partial repair,
and payload-free UI projection. Token accounting, range selection, summary
dispatch, checkpoint construction, tool-result pruning, and the real Agent
producer remain a later Phase 8 slice.
...
but the Phase 8 compaction row must not become `compatible` until the real Agent
producer, token accounting, pruner, and deterministic comparison tests are all
present.
