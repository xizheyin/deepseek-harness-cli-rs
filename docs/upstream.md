...
and starts no model stream until that generation fits. Focused Rust tests also
lock the frozen camel-case wire shape, reader-only
...
The output-usage anchor, pressure trigger, summary dispatch/checkpoint
construction, and context-overflow replay remain later Phase 8 slices.
