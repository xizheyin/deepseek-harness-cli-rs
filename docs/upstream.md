...
resident. Durable claim settlement now also moves one exact `PreparedEvent`
between the caller's claim and the Session-owned pending operation instead of
deep-cloning it across an await. Clock rejection, injected clock panic, writer
poison, cancelled waits, and shutdown are covered as distinct owner transfers;
the Agent-level panic regression proves that the reserved `step/end` and
`turn/end` still close. A preferred-only result produced after an irreversible
tool side effect is never replaced by its fallback: one transient Clock failure
retries the exact Session-owned candidate, with durable and memory-mode error
classification tested separately. A second Clock rejection preserves that root
error and leaves the exact candidate Session-owned for shutdown/recovery rather
than obscuring it with `NeedsAppendSettle`. Attempt payloads, claim resident credit,
cold-recovery work, and closure headroom are not charged yet, so the complete
shared 32 MiB invariant, pressure trigger, summary dispatch/checkpoint
construction, and Agent-level context-overflow interception and replay remain
...
Rust substrate; it does not promote the still-unreachable Agent producer.

The generated oracle remains the comparison input for the unfinished producer
work. The Rust event/projection/recovery substrate may be recorded as `partial`,
but the Phase 8 compaction row must not become `compatible` until the pressure,
summary, and overflow-replay producer paths plus their deterministic comparison
